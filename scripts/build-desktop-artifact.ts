...
const DESKTOP_APP_ID = "com.t3tools.t3code.kanban";
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ...
    : (desktopPackageJson.productName ?? "T3 Code Kanban");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "T3-Code-Kanban-${version}-${arch}.${ext}",
  ...
  if (platform === "mac") {
    ...
    buildConfig.mac = {
      ...
      protocols: [
        {
          name: "T3 Code Kanban",
          schemes: ["t3code-kanban", "t3code-kanban-dev"],
        },
      ],
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "t3code-kanban",
      icon: "icons",
      category: "Development",
      // electron-builder turns these into MimeType=x-scheme-handler/<scheme>;
      // in the .desktop entry (Exec already gets %U), so browsers can hand
      // t3code-kanban:// OAuth callbacks to the app.
      protocols: [
        {
          name: "T3 Code Kanban",
          schemes: ["t3code-kanban", "t3code-kanban-dev"],
