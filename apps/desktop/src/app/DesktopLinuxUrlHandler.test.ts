...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    isPackaged: true,
    isDevelopment: false,
    displayName: "T3 Code Kanban (Alpha)",
    linuxDesktopEntryName: "com.t3tools.T3CodeKanban.desktop",
    linuxWmClass: "t3code-kanban",
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    const entry = DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
      displayName: "T3 Code Kanban (Nightly)",
      execTarget: '/home/al ice/Apps/T3 "100%" $HOME\\x.AppImage',
      scheme: "t3code-kanban",
    });

    assert.include(entry, "[Desktop Entry]");
    assert.include(entry, "Name=T3 Code Kanban (Nightly)");
    ...
    assert.include(entry, "MimeType=x-scheme-handler/t3code-kanban;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "t3code-kanban",
      desktopEntryPath: "/home/alice/.local/share/applications/com.t3tools.T3CodeKanban.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the t3code-kanban:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/com.t3tools.T3CodeKanban.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the t3code-kanban:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/t3code-kanban;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "com.t3tools.T3CodeKanban.desktop", "x-scheme-handler/t3code-kanban"],
        },
      ]);
    });
  });
  ...
  it.effect("writes the portal identity without claiming the URL scheme in development", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(
        ...
        "/home/alice/.local/share/applications/com.t3tools.T3CodeKanban.Development.desktop",
      );
